...
app.get("/scholarship-page", (req, res) => {
  res.sendFile(path.join(__dirname, "Views", "scholarship.html"));
});
...
app.patch("/api/user/:userId", async (req, res) => {
  try {
    ...
    if (!updatedUserId) return res.status(404).json({ msg: "User Not Found" });
    res.status(200).json({
...
app.patch("/api/jobs/:jobId", async (req, res) => {
  try {
    ...
    if (!updatedJobId) return res.status(404).json({ msg: "Job Not Found" });
    res.status(200).json({
...
app.get("/api/scholarships/:scholarId", async (req, res) => {
  try {
    ...
    const foundScholar = await scholarship.findOne({
...
app.delete("/api/scholarships/:scholarId", async (req, res) => {
  try {
    ...
    const deletedScholarship = await scholarship.deleteOne({
      scholarshipId: deletedId,
    });
...
app.patch("/api/scholarships/:scholarshipId", async (req, res) => {
  ...
  const updatedScholarship = await scholarship.findOneAndUpdate(
    { scholarshipId: updatedScholarshipId },
    req.body,
    { new: "true" },
  );
